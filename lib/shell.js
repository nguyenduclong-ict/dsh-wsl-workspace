import { c as joinUnc, d as parseWslUnc, f as windowsToMntPath, n as defaultDistroSync, o as isValidWslUsername, s as isWindowsPathShaped } from "./wsl-CFAVh18Z.js";
import { r as getWorkspaceUsername } from "./wsl-credentials-CC-J8zLX.js";
...
var WslShellExecutor = class WslShellExecutor extends ShellExecutor {
	...
	*/
	plan(spec) {
		...
		const command = login || interactive ? `cd '${linuxCwd.replace(/'/g, `'\\''`)}' && ${spec.command}` : spec.command;
		const argv = [
			this.config.wslPath,
			"-d",
			distro,
